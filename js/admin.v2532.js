...
(function(){
  ...
  // 把文件转 base64（v25.32: 3MB 上限，支持 Promise）
  ...
  // Promise 版本，form submit 时 await 此函数确保 base64 完成
  function _fileToDataURLP(input) {
    return new Promise(function(resolve){
      _fileToDataURL(input, resolve);
    });
  }
  function _attachFileUpload(form) {
    // 给 data-f=image_url 的 input / textarea 加 file 选择
    ...
    fields.forEach(function(inp){
      ...
      inp.placeholder = '图片 URL 或选文件上传';
      // input 本身是 url 框，file 选择跟在后面
      var file = document.createElement('input');
      file.type = 'file';
      file.accept = 'image/*';
      file.name = '_file_' + inp.dataset.f;
      file.style.cssText = 'display:block;width:100%;margin-top:4px;padding:4px 6px;border:2px solid var(--c-black);font-size:11px;background:var(--c-bg-2);cursor:pointer;';
      file.title = '点击选择图片文件（上限 3MB）';
