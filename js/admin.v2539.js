...
// v25.39: 改用 .ae-* class (暗色 popup, 独立于主后台视觉)
function showAdminEnterModal(player, adminId) {
  ...
  mask.className = 'ae-mask';
  mask.innerHTML = `
    <div class="ae-card">
      <h3 class="ae-title">🛡️ 进入管理后台</h3>
      <p class="ae-desc">
        当前已登录玩家 <b class="ae-username">${esc(player.username)}</b>。<br>
        进入管理需要 <b class="ae-highlight">二级验证</b>: 输入关联管理员的密码, 或用通行密钥。
      </p>
      <div id="adminEnterTabs" class="ae-tabs">
        <button id="tabPw" class="enter-tab enter-tab-active">🔑 管理员密码</button>
        <button id="tabPk" class="enter-tab">🔐 通行密钥</button>
      </div>
      <div id="adminEnterPw">
        <label class="ae-label">关联管理员密码</label>
        <input type="password" id="adminEnterPwInput" placeholder="管理员密码 (至少 8 位)" class="ae-input">
        <div id="adminEnterMsg" class="ae-msg"></div>
        <div class="ae-row">
          <button class="btn btn-ghost btn-sm" id="adminEnterCancel">取消</button>
          <button class="btn btn-primary btn-sm" id="adminEnterPwBtn">✓ 进入管理</button>
        </div>
      </div>
      <div id="adminEnterPk" class="ae-pk" style="display:none;">
        <p class="ae-desc-sm">使用你已注册的通行密钥验证身份</p>
        <div id="adminEnterPkMsg" class="ae-msg"></div>
        <div class="ae-row">
          <button class="btn btn-ghost btn-sm" id="adminEnterCancel2">取消</button>
          <button class="btn btn-primary btn-sm" id="adminEnterPkBtn">🔐 用通行密钥进入</button>
        </div>
      </div>
      <div class="ae-footer">
        <a href="/" class="ae-back">← 返回首页 (玩家身份保持登录)</a>
      </div>
    </div>
  `;
...
function showAdminPasskeyOffer(adminId,dismissKey){
  ...
  bd.className='apko-anchor';
  bd.innerHTML=`
    <div class="apko-bar apko-card">
      <div class="apko-bar-head">
        <span class="apko-emoji">🔑</span>
        <div class="grow">
          <b class="apko-title">管理员要不要也加一个通行密钥？</b>
          <div class="apko-sub">下次可指纹 / Face ID 一键进后台，不用记密码</div>
        </div>
        <button type="button" id="apkoClose" aria-label="关闭" class="apko-close" title="关闭">×</button>
      </div>
      <div class="apko-row">
        <button type="button" id="apkoAdd" class="apko-btn apko-btn-primary">✅ 立即添加</button>
        <button type="button" id="apkoLater" class="apko-btn apko-btn-ghost">⏭ 下次再说</button>
      </div>
      <div id="apkoMsg" class="apko-bar-msg"></div>
    </div>
  `;
...
async function renderMessages(){
  try{
    ...
    box.innerHTML=list.map(m=>{
      ...
      let chainTag;
      if(!hasReply){
        chainTag='<span class="msg-replied-tag tag-pending">⏳ 待回复</span>';
      }else if(hasPreviousAi){
        chainTag='<span class="msg-replied-tag tag-ai-overridden" title="AI 先自动回复，后被管理员覆盖">🤖→💬 已被人工覆盖</span>';
      }else if(isAiReply){
        chainTag='<span class="msg-replied-tag tag-ai-replied">🤖 AI 已回复</span>';
      }else{
        chainTag='<span class="msg-replied-tag tag-human-replied">💬 人工已回复</span>';
      }
...
function showReplyModal(m){
  ...
  bd.className='ae-mask';
  const fmtDate=s=>{try{return new Date(s*1000).toLocaleString('zh-CN',{hour12:false});}catch(_){return String(s||'');}};
  const context=m.content?`<div class="ae-context">${esc(m.content)}</div>`:'';
  const contact=m.contact?`<div class="ae-contact">📇 联系方式：${esc(m.contact)}</div>`:'';
  const nameTag=m.name?`<div class="ae-name-row">👤 ${esc(m.name)} <span class="muted">· ${fmtDate(m.created_at)}</span></div>`:'';

  bd.innerHTML=`
    <div class="ae-card" style="max-width:560px;">
      <h3 class="ae-title">💬 回复留言 #${m.id}</h3>
      ${nameTag}
      ${contact}
      ${context}
      ${m.admin_reply && m.admin_reply.startsWith('🤖')?'<div class="hint hint-ai">🤖 AI 已自动回复，管理员可编辑覆盖</div>':''}
      <div class="form-field">
        <textarea id="replyText" placeholder="输入回复内容…" class="ae-textarea">${esc(m.admin_reply||'')}</textarea>
      </div>
      <div class="ae-actions">
        <span class="grow"></span>
        <div class="ai-tone-row">
          <button id="aiStandardBtn" type="button" class="ai-tone-btn ai-standard" title="亲切礼貌 100 字内">🤖 AI 草稿</button>
          <button id="aiProBtn" type="button" class="ai-tone-btn ai-pro" title="政府公文体 100 字内">💼 专业回复</button>
          <button id="aiDetailBtn" type="button" class="ai-tone-btn ai-detail" title="5 段结构 300-500 字详尽回复">📋 详细回复</button>
          <button id="aiShortBtn" type="button" class="ai-tone-btn ai-short" title="30 字内极简">⚡ 极简</button>
        </div>
        <button id="replyCancel" type="button" class="btn btn-ghost btn-sm">取消</button>
        <button id="replyClear" type="button" class="btn btn-soft-danger btn-sm">清空</button>
        <button id="replySave" type="button" class="btn btn-primary btn-sm">保存</button>
      </div>
    </div>
  `;
  document.body.appendChild(bd);
...
async function renderPlayers(){
  try{
    ...
    box.innerHTML=shown.map(p=>{
      ...
      const lastSession=p.last_session?fmt(p.last_session):'<i class="muted">从未登录</i>';
      return `<article class="msg-item" data-id="${p.id}">
        <div class="msg-head"><div class="msg-head-left">
          <b class="msg-name">${esc(p.avatar_emoji||'👤')} ${esc(p.username)}</b>
          <span class="player-card-meta">${esc(p.email)}</span>
          <span class="msg-player-tag">${STATUS_LABEL[p.status]||p.status}</span>
          ${p.game_id?`<span class="gallery-num player-card-gameid" title="游戏ID">🎮 ${esc(p.game_id)}</span>`:''}
        ...
        <p class="msg-content player-card-bio">${p.bio?esc(p.bio):'<i>暂无简介</i>'}</p>
        <div class="msg-actions book-actions">
...
function showCreatePlayerModal(){
  ...
  bd.className='ae-mask';
  bd.innerHTML=`
    <div class="ae-card" style="max-width:480px;">
      <h3 class="ae-title">🆕 代注册玩家账号</h3>
      <p class="ae-desc">
        由 super 管理员直接创建账号，无需玩家本人注册和审批。账号立即激活可用。
      </p>
      <div class="ae-grid">
        <label class="ae-form-label">
          <span>玩家用户名 <b class="ae-required">*</b> <small>（2-32 字符，中文/字母/数字/下划线/连字符/点/空格，不含 @）</small></span>
          <input id="cpUser" type="text" placeholder="如：SIM_漫画家" class="ae-input ae-input-md">
        </label>
        <label class="ae-form-label">
          <span>邮箱 <b class="ae-required">*</b></span>
          <input id="cpEmail" type="email" placeholder="player@example.com" class="ae-input ae-input-md">
        </label>
        <label class="ae-form-label">
          <span>游戏 ID <small>（可选）</small></span>
          <input id="cpGame" type="text" placeholder="Minecraft 游戏内 ID" class="ae-input ae-input-md">
        </label>
        <label class="ae-form-label">
          <span>初始密码 <b class="ae-required">*</b> <small>（至少 8 位）</small></span>
          <input id="cpPass" type="text" placeholder="可填临时密码，玩家可自行修改" class="ae-input ae-input-md">
        </label>
      </div>
      <div id="cpMsg" class="ae-msg"></div>
      <div class="ae-actions">
        <button id="cpCancel" type="button" class="btn btn-ghost">取消</button>
        <button id="cpSave" type="button" class="btn btn-primary">✓ 创建账号</button>
      </div>
    </div>
  `;
  document.body.appendChild(bd);
...
function bookEdit(b) {
  ...
  bd.className = 'ae-mask';
  bd.innerHTML = `
    <div class="ae-card ae-card-yellow" style="max-width:540px;">
      <h3 class="ae-title ae-title-yellow">✎ 编辑酒店预订 #${b.id}</h3>
      <div class="form-field">
        <span class="form-field-label">姓名</span>
        <input id="beName" value="${esc(b.name||'')}" class="ae-form-input">
      </div>
      <div class="form-field">
        <span class="form-field-label">联系方式</span>
        <input id="beContact" value="${esc(b.contact||'')}" class="ae-form-input">
      </div>
      <div class="form-row">
        <label class="form-field">
          <span class="form-field-label">入住日期</span>
          <input id="beIn" type="date" value="${esc(b.in_date||'')}" class="ae-form-input">
        </label>
        <label class="form-field">
          <span class="form-field-label">退房日期</span>
          <input id="beOut" type="date" value="${esc(b.out_date||'')}" class="ae-form-input">
        </label>
      </div>
      <div class="form-row">
        <label class="form-field">
          <span class="form-field-label">人数</span>
          <input id="bePersons" type="number" min="1" value="${b.persons||1}" class="ae-form-input">
        </label>
      ...
      <div class="form-field">
        <span class="form-field-label">特殊要求</span>
        <textarea id="beNote" rows="2" class="ae-form-textarea">${esc(b.note||'')}</textarea>
      </div>
      <div class="ae-actions">
        <button id="beCancel" type="button" class="btn btn-ghost">取消</button>
        <button id="beSave" type="button" class="btn btn-gold">💾 保存</button>
      </div>
    </div>
  `;
  document.body.appendChild(bd);
  ...
  try{
    ...
    box.innerHTML=shown.map(x=>`<article class="msg-item" data-id="${x.id}">
      <div class="msg-head"><div class="msg-head-left">
        ...
        <span class="player-card-meta">${esc(x.contact||'')}</span>
      ...
      ${x.note?`<div class="msg-content text-sm">📝 ${esc(x.note)}</div>`:''}
      <div class="msg-actions book-actions">
...
function kartCircuitEdit(it, kind) {
  ...
  const _colorClass = isK ? 'ae-card-yellow' : 'ae-card-purple';
  const _titleClass = isK ? 'ae-title-yellow' : 'ae-title-purple';
  const _saveBtn = isK ? 'btn-gold' : 'btn-purple';
  ...
  bd.innerHTML = `
    <div class="ae-card ${_colorClass}" style="max-width:540px;">
      <h3 class="ae-title ${_titleClass}">✎ 编辑${_type} #${it.id}</h3>
      <div class="form-row">
        <div class="form-field">
          <span class="form-field-label">姓名</span>
          <input id="kcName" value="${esc(it.name||'')}" class="ae-form-input">
        </div>
        <div class="form-field">
          <span class="form-field-label">联系方式</span>
          <input id="kcContact" value="${esc(it.contact||'')}" class="ae-form-input">
        </div>
      </div>
      <div class="form-row">
        <div class="form-field">
          <span class="form-field-label">场次 (session)</span>
          <input id="kcSession" value="${esc(it.session||'')}" class="ae-form-input">
        </div>
        <div class="form-field">
          <span class="form-field-label">${isK ? '车辆 (car)' : '驾照 (license)'}</span>
          <input id="kcCar" value="${esc((isK?it.car:it.license)||'')}" class="ae-form-input">
        </div>
      </div>
      <div class="form-field">
        <span class="form-field-label">状态</span>
        <select id="kcStatus" class="ae-form-input">${_statusOpts}</select>
      </div>
      <div class="form-field">
        <span class="form-field-label">备注</span>
        <textarea id="kcNote" rows="2" class="ae-form-textarea">${esc(it.note||'')}</textarea>
      </div>
      <div class="ae-actions">
        <button id="kcCancel" type="button" class="btn btn-ghost">取消</button>
        <button id="kcSave" type="button" class="btn ${_saveBtn}">💾 保存</button>
      </div>
    </div>
  `;
  document.body.appendChild(bd);
  ...
  try{
    ...
    box.innerHTML=list.map(a=>{
      ...
      const linkedBadge = a.linked_player_id
        ? `<span class="tag tag-purple" title="此管理员已与该玩家账号合并: 用玩家密码或通行密钥即可登入管理; 退出管理无需重新验证">🔗 合并: ${esc(a.linked_player_username||'#'+a.linked_player_id)}</span>`
        : '';
...
function adminEdit(a) {
  ...
  bd.innerHTML = `
    <div class="ae-card ae-card-purple" style="max-width:440px;">
      <h3 class="ae-title ae-title-purple">✎ 编辑管理员 #${a.id}</h3>
      <div class="form-field">
        <span class="form-field-label">用户名 (3-32 字符)</span>
        <input id="admUser" value="${esc(a.username)}" ${isMe?'disabled':''} class="ae-form-input ${isMe?'ae-form-input-disabled':''}">
      </div>
      <div class="form-field">
        <span class="form-field-label">角色</span>
        <select id="admRole" ${isMe?'disabled':''} class="ae-form-input ${isMe?'ae-form-input-disabled':''}">
          <option value="admin" ${a.role==='admin'?'selected':''}>ADMIN (普通管理员)</option>
          <option value="super" ${a.role==='super'?'selected':''}>SUPER (超级管理员)</option>
        </select>
      </div>
      ${isMe?'<p class="ae-warn">⚠️ 自己的账号不能改用户名/角色, 用"修改密码"按钮改密码</p>':''}
      <div class="ae-actions">
        <button id="admCancel" type="button" class="btn btn-ghost">取消</button>
        <button id="admSave" type="button" class="btn btn-purple">💾 保存</button>
      </div>
    </div>
  `;
  document.body.appendChild(bd);
...
async function showMergePlayerModal(adminId) {
  ...
  mask.innerHTML = `
    <div class="modal" style="max-width:520px;">
      ...
      <div class="modal-body">
        <p class="mp-hint">合并后: 用该玩家密码或通行密钥登录, 即可同时获得管理身份; 退出管理无需重新验证, 玩家身份保留。两边密码独立, 互不影响。</p>
        <div class="form-field">
          <input type="text" id="mpSearch" placeholder="搜索玩家 username / game_id / email..." class="mp-search">
        </div>
        <div id="mpList" class="mp-list">
          <p class="mp-list-status">载入中…</p>
        </div>
      </div>
    </div>
  `;
  ...
  try {
    ...
    document.getElementById('mpList').innerHTML = '<p class="mp-list-status mp-list-error">加载失败: ' + esc(e.message) + '</p>';
    return;
  }
  ...
  function render(q) {
    ...
    if (!filtered.length) {
      list.innerHTML = '<p class="mp-list-status">无匹配玩家</p>';
      return;
    }
    list.innerHTML = filtered.map(p => `
      <div class="mp-row" data-pid="${esc(String(p.id))}" data-pname="${esc(p.username)}">
        <span class="mp-row-emoji">${esc(p.avatar_emoji || '👤')}</span>
        <div class="grow">
          <div class="mp-row-name">${esc(p.username)}</div>
          <div class="mp-row-meta">ID: ${p.id} · ${esc(p.game_id || '-')} · ${esc(p.email || '')}</div>
        </div>
        <span class="mp-row-act">→ 合并</span>
      </div>
    `).join('');
...
async function safeRender(fn){try{await fn();}catch(e){console.error(e);}}
    ...
    try {
      ...
      list.innerHTML = anns.map(a => `
        <article class="msg-item ann-item" data-id="${a.id}">
          ${a.image_url ? `<img src="${esc(a.image_url)}" alt="封面图" loading="lazy" class="cover-img" onerror="this.style.opacity=0" />` : ''}
          <div class="msg-head"><div class="msg-head-left">
            <b class="msg-name">📢 ${esc(a.title)}</b>
            <span class="msg-read-tag">仅 SUPER</span>
          </div><div class="msg-time">${fmt(a.created_at)}${a.updated_at ? ' <span class="muted">· 已编辑</span>' : ''}</div></div>
          <div class="msg-content pre-wrap">${esc(a.content)}</div>
      ...
      list.innerHTML = '<p class="list-error">✗ ' + esc(e.message) + '</p>';
    }
  }
        ...
        <div class="modal-body">
          <form id="annForm" class="modal-form" onsubmit="return false;">
            ...
            <label>
              ...
              <textarea id="annContent" maxlength="2000" placeholder="公告正文..." class="ann-content-textarea">${esc(ann ? ann.content : '')}</textarea>
              <span id="annCount" class="ann-count">0 / 2000</span>
            </label>
            <label>
  ...
  async function renderGallery() {
    ...
    try {
      ...
      grid.innerHTML = list.map(it => `
        <div class="gal-item ${it.is_published ? 'is-published' : 'is-draft'}" data-id="${it.id}">
          <div class="gal-item-img">
            <img src="${esc(it.file_url)}" alt="${esc(it.label)}" loading="lazy" onerror="this.style.opacity=.3;this.alt='加载失败'" />
          </div>
          <div class="gal-item-body">
            <div class="gal-item-title">#${it.num} ${esc(it.label)}</div>
            <div class="gal-item-meta">${esc(it.cat)}${it.is_featured ? ' · ⭐精选' : ''}${it.is_published ? '' : ' · ⛔草稿'}</div>
            <div class="gal-item-tools">
              <button class="btn btn-ghost btn-sm gal-edit" data-id="${it.id}">✎</button>
              <button class="btn btn-ghost btn-sm gal-toggle" data-id="${it.id}">${it.is_published ? '下架' : '上架'}</button>
              <button class="btn btn-ghost btn-sm gal-feat" data-id="${it.id}">${it.is_featured ? '取消精选' : '⭐精选'}</button>
              <button class="btn btn-ghost btn-sm gal-del btn-soft-danger" data-id="${it.id}">🗑</button>
            </div>
          </div>
        </div>
      `).join('');
      ...
      grid.innerHTML = '<p class="list-error">✗ ' + esc(e.message) + '</p>';
    }
  }
  ...
  function showGalModal(item) {
    ...
    bd.innerHTML = `
      <div class="ae-card" style="max-width:560px;">
        <h3 class="ae-title">${isNew ? '🖼️ 添加首页图片' : '✎ 编辑图片 #' + item.id}</h3>
        <div class="form-field">
          <span class="form-field-label">编号 (1-999, 唯一, 控制显示顺序)</span>
          <input id="galNum" type="number" min="1" max="999" value="${_v('num')}" class="ae-form-input">
        </div>
        <div class="form-field">
          <span class="form-field-label">分类</span>
          <select id="galCat" class="ae-form-input">
        ...
        <div class="form-field">
          <span class="form-field-label">标题 (label, 80 字内)</span>
          <input id="galLabel" maxlength="80" value="${_v('label')}" placeholder="如：市中心发言台" class="ae-form-input">
        </div>
        <div class="form-field">
          <span class="form-field-label">图片 (二选一)</span>
          <div class="gal-file-row">
            <input id="galFile" type="file" accept="image/*" class="ae-file-input">
            <span class="muted text-tiny">或粘贴 URL ↓</span>
          </div>
          <textarea id="galUrl" maxlength="2000" placeholder="https://... 或 data:image/png;base64,..." class="ae-form-textarea" style="min-height:80px;font-family:monospace;">${_v('file_url')}</textarea>
          <div id="galPreview" class="gal-preview" style="display:${_v('file_url') ? 'block' : 'none'};">
            <img id="galPreviewImg" src="${_v('file_url')}" class="gal-preview-img">
          </div>
        </div>
        <div class="form-row">
          <label class="ae-checkbox-row" style="flex:0 0 auto;">
            排序 (小在前)
            <input id="galSort" type="number" value="${_v('sort_order') || 0}" class="ae-form-input" style="width:80px;padding:6px 8px;">
          </label>
          <label class="ae-checkbox-row" style="flex:0 0 auto;padding-top:0;">
        ...
        <div class="ae-actions">
          <button id="galCancel" type="button" class="btn btn-ghost">取消</button>
          <button id="galSave" type="button" class="btn btn-cyan">${isNew ? '🖼️ 添加' : '💾 保存'}</button>
        </div>
      </div>
    `;
    document.body.appendChild(bd);
...
async function renderDms(query) {
  ...
  try {
    ...
    list.innerHTML = convs.map(p => {
      ...
      if (isAiReply) {
        if (p.replied_by_admin_id && p.replied_by_admin_username) {
          // 管理员借 AI 身份代发
          replyTag = `<span class="tag tag-info" style="margin-left:6px;" title="管理员 ${esc(p.replied_by_admin_username)} 借灯灯客服身份代发">👤 管理员 ${esc(p.replied_by_admin_username)} 已回复</span>`;
        } else {
          // AI 客服自动回复
          replyTag = '<span class="tag tag-success" style="margin-left:6px;">🤖 AI 已回复</span>';
        }
      }
      const unreadTag = unread>0?`<span class="tag tag-danger" style="margin-left:6px;">💬 ${unread} 未读</span>`:'';
      return `<div class="msg-item dm-pair ${unread>0?'is-unread':''}" data-pid1="${p.from_player_id}" data-pid2="${p.to_player_id}">
        ...
        <div class="msg-body">
          <div class="msg-meta">
            ...
            ${replyTag}
            ${unreadTag}
            <span class="dm-pair-tail">${p.last_at}</span>
          </div>
          <div class="msg-content dm-pair-preview">${esc((p.last_content||'').slice(0,200))}</div>
        </div>
      </div>`;
    }).join('');
    ...
    list.innerHTML = '<p class="empty-state list-error">✗ ' + esc(e.message) + '</p>';
  }
}
...
async function openDmThread(pid1, pid2) {
  ...
  mb.innerHTML = '<p class="dm-thread-loading">载入中…</p>';
  md.style.display = '';
  try {
    // 拿双方 username
    ...
    mb.innerHTML = `
      <div class="dm-thread">
        ${msgs.length===0?'<p class="dm-thread-empty">（暂无消息）</p>':''}
        ${msgs.map(m => {
          const isBot = m.from_username === '灯灯客服';
          return `<div class="dm-bubble ${isBot?'is-bot':''}">
            <div class="dm-bubble-bubble">
              <div class="dm-bubble-meta">${esc(m.from_username||'?')} · ${m.created_at}</div>
              <div class="dm-bubble-text">${esc(m.content||'')}</div>
            </div>
          </div>`;
        }).join('')}
      </div>
      ${botId ? `
      <div class="dm-reply-block">
        <div class="dm-reply-hint">✍️ 借 <b>灯灯客服</b> 身份回复给 <b>${esc(humanName)}</b>：</div>
        <textarea id="dmReplyText" class="dm-reply-textarea" placeholder="输入回复内容（最多 100 字）..." maxlength="100"></textarea>
        <div class="dm-reply-count"><span id="dmReplyCount">0</span> / 100 字</div>
        <div class="ae-actions" style="margin-top:8px;">
          <button type="button" id="dmAiBtn" class="btn apko-btn-primary btn-sm" title="AI 辅助生成 100 字内回复草稿">🤖 AI 辅助生成</button>
        ...
        <div id="dmReplyMsg" class="dm-reply-msg"></div>
      </div>
      ` : '<p class="dm-reply-na">此对话双方都是普通玩家，无法借 AI 身份回复。请直接联系玩家。</p>'}
    `;
    // 字数计数
    ...
    mb.innerHTML = '<p class="dm-error">✗ ' + esc(e.message) + '</p>';
  }
}
...
async function renderDmAiStruggle() {
  ...
  try {
    ...
    if (ss.length === 0) {
      list.innerHTML = '<p class="empty-state dm-empty-celebrate">🎉 AI 没有转人工的对话</p>';
      return;
    }
    list.innerHTML = '<p class="dm-empty-hint">⚠️ 以下是 AI 客服给出"转人工"建议的对话，玩家可能需要人工协助。点击查看完整对话：</p>' + ss.map(s => {
      return `<div class="msg-item dm-pair dm-struggle" data-pid1="${s.from_player_id}" data-pid2="${s.to_player_id}">
        <div class="msg-avatar">🤖</div>
        <div class="msg-body">
          <div class="msg-meta">
            <b>${esc(s.to_username || '?')}</b> → <b>灯灯客服 (AI 转人工)</b>
            <span class="dm-pair-tail">${s.created_at}</span>
          </div>
          <div class="msg-content dm-struggle-preview">${esc((s.content||'').slice(0,200))}</div>
        </div>
      </div>`;
    }).join('');
    list.querySelectorAll('.dm-pair').forEach(el => {
    ...
    list.innerHTML = '<p class="empty-state dm-error">✗ ' + esc(e.message) + '</p>';
  }
}
...
(function(){
  ...
  function _buildInlineForm(opts) {
    var fields = opts.fields.map(function(f){
      return '<label class="inline-form-field">' +
        '<span class="inline-form-label">' + esc2(f.label) + '</span>' +
        (f.type === 'textarea' ? '<textarea data-f="' + f.name + '" rows="3" class="inline-form-textarea">' + esc2(f.value || '') + '</textarea>'
          : '<input data-f="' + f.name + '" type="' + (f.type || 'text') + '" value="' + esc2(f.value || '') + '" class="inline-form-input" />') +
        '</label>';
    }).join('');
    return '<div class="inline-form" data-form-wrap="' + esc2(opts.formKey || 'inline') + '">' +
      '<div class="inline-form-title">' + esc2(opts.title) + '</div>' +
      fields +
...
(function(){
  ...
  function _editForm(opts) {
    ...
    var fields = opts.fields.map(function(f){
      return '<label class="inline-form-field">' +
        '<span class="inline-form-label">' + esc3(f.label) + (f.required ? ' *' : '') + '</span>' +
        (f.type === 'textarea' ? '<textarea data-f="' + f.name + '" rows="3" class="inline-form-textarea">' + esc3(f.value || '') + '</textarea>'
          : '<input data-f="' + f.name + '" type="' + (f.type || 'text') + '" value="' + esc3(f.value || '') + '"' + (f.required ? ' required' : '') + ' class="inline-form-input" />') +
        '</label>';
    }).join('');
    return '<div class="inline-form" data-form-wrap="' + esc3(opts.formKey) + '">' +
      '<div class="inline-form-title">' + esc3(opts.title) + '</div>' +
      fields +
      '<div class="inline-form-actions">' +
        '<button data-act="save" class="btn btn-primary btn-sm">💾 保存</button>' +
        '<button data-act="cancel" class="btn btn-ghost btn-sm">取消</button>' +
  ...
  async function renderHotelManage(){
    ...
    list.innerHTML = hotels.map(function(h){
      ...
      return '<article class="msg-item ann-item">' +
        ...
        (h.image_url ? '<div><img class="cover-thumb" src="' + esc(h.image_url) + '" loading="lazy" onerror="this.style.opacity=\'0.25\'" alt="酒店图片" /></div>' : '') +
        (h.address ? '<div class="msg-content">📍 ' + esc(h.address) + '</div>' : '') +
        (h.description ? '<div class="msg-content pre-wrap">' + esc(h.description) + '</div>' : '') +
        (hrs.length > 0 ? '<div class="rooms-block">' +
          hrs.map(function(r){
            return '<div class="room-row">' +
              '<span class="grow">🛏️ ' + esc(r.name) + ' · ' + r.capacity + '人 · ' + (r.breakfast_included ? '🍳' : '—') + '</span>' +
              '<span class="room-row-price">💎 ' + r.price_per_night + '/晚</span>' +
              (_me ? '<button data-act="edit-room" data-id="' + r.id + '" class="btn btn-ghost btn-sm btn-xs">✎</button>' +
              '<button data-act="del-room" data-id="' + r.id + '" class="btn btn-ghost btn-sm btn-xs">🗑</button>' : '') +
              '</div>';
          }).join('') + '</div>' : '') +
        (_me ? '<div class="msg-actions book-actions">' +
          ...
          '<button data-act="del-hotel" data-id="' + h.id + '" class="btn btn-ghost btn-sm btn-soft-danger">🗑 删除</button>' +
        '</div>' : '') +
        '</article>';
    }).join('');
  }
  ...
  async function renderTrackManage(){
    ...
    list.innerHTML = tracks.map(function(t){
      return '<article class="msg-item ann-item">' +
        ...
        (t.image_url ? '<div><img class="cover-thumb-noborder" src="' + esc(t.image_url) + '" loading="lazy" onerror="this.style.opacity=\'0.25\'" alt="赛道图片" /></div>' : '') +
        (t.difficulty ? '<div class="msg-content">难度: ' + esc(t.difficulty) + '</div>' : '') +
        (t.description ? '<div class="msg-content pre-wrap">' + esc(t.description) + '</div>' : '') +
        (_me ? '<div class="msg-actions book-actions">' +
          '<button data-act="edit-track" data-id="' + t.id + '" class="btn btn-primary btn-sm">✎ 编辑</button>' +
          '<button data-act="del-track" data-id="' + t.id + '" class="btn btn-ghost btn-sm btn-soft-danger">🗑 删除</button>' +
        '</div>' : '') +
        '</article>';
    }).join('');
  }
  ...
  async function renderLicenseManage(){
    ...
    list.innerHTML = reqs.map(function(l){
      return '<article class="msg-item ann-item">' +
        ...
        (l.description ? '<div class="msg-content pre-wrap">' + esc(l.description) + '</div>' : '') +
        (l.requirements ? '<div class="msg-content hint-warn">📋 ' + esc(l.requirements) + '</div>' : '') +
        (_me ? '<div class="msg-actions book-actions">' +
          '<button data-act="edit-license" data-id="' + l.id + '" class="btn btn-primary btn-sm">✎ 编辑</button>' +
          '<button data-act="del-license" data-id="' + l.id + '" class="btn btn-ghost btn-sm btn-soft-danger">🗑 删除</button>' +
        '</div>' : '') +
        '</article>';
    }).join('');
  }
