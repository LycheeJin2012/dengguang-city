...
export async function render(el){
  ...
  el.innerHTML=title('市民档案','Citizen profile')+`<section class="pane pane-pad-lg"><div class="pane-head"><div class="room-status"><span class="avatar">${esc(p.avatar_emoji||'👤')}</span><h2 style="margin:0">${esc(p.username)}</h2></div><span class="badge">${tr('加入','Joined')} ${days} ${tr('天','days')}</span></div><p>${text(p.bio||tr('这位市民还没有填写简介。','This citizen has not added a bio.'))}</p><small>${tr('注册时间','Registered')} ${date(p.created_at)}</small><div class="stat-grid" style="margin:16px 0"><div class="stat-tile"><strong>${d.stats.messages}</strong><span>${tr('留言','Messages')}</span></div><div class="stat-tile"><strong>${d.stats.comments}</strong><span>${tr('评论','Comments')}</span></div></div><div class="actions">${self?`<a class="button" href="/affairs.html">我的事务</a><button id="profile-passkeys">${tr('管理 / 验证通行密钥','Manage / verify passkeys')}</button><button id="edit-profile">${tr('编辑资料','Edit profile')}</button><button id="citizen-card">${tr('下载市民卡','Download citizen card')}</button><button id="daily-signin" class="cta-pulse">🎁 ${tr('签到','Check in')}</button>`:`<a class="button" href="/dm.html?to=${encodeURIComponent(p.username)}">${tr('发送私信','Send a message')}</a>`}</div></section>${self?`<div class="tabs-segment" role="tablist" id="profile-tabs" aria-label="${tr('个人资料','Profile')}">${[['history','我的记录','My records'],['security','账号与登录','Account & sign-in'],['race','赛道成绩','Race times'],['exam','模拟考试','Practice exam'],['subscriptions','通知订阅','Subscriptions'],['rewards','工单奖励','Ticket rewards']].map(([k,zh,en])=>`<button type="button" role="tab" data-tab="${k}" aria-selected="${k==='history'}">${tr(zh,en)}</button>`).join('')}</div><section id="profile-content" class="pane pane-pad-lg" style="margin-top:24px"></section>`:''}`;
  if(!self)return;
  ...
  async function tab(key){
    ...
    box.className='pane pane-pad-lg';
