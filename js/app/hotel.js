...
={
  ...
  el.innerHTML=rooms.length?`<div class="feature-grid">${rooms.map(r=>{
...
export async function render(el){
  el.innerHTML=title('树上酒店','Treehouse Hotel')+`<div class="toolbar-modern">${field('availability',tr('状态','Status'),'select','all',{
  ...
  )}</div><div id="rooms" class="card-list" style="margin-top:24px"></div>`;
  await region($('#rooms',el),()=>api('/api/homepage-bundle'),(d,box)=>{
    ...
    ;$$('.toolbar-modern input,.toolbar-modern select',el).forEach(x=>x.onchange=draw);draw();
  }
  );
}
