...
// 工厂函数: 每次 render 取当前语言的 type label (跟 STATUS_LABEL 模式一致)
const typeLabel = (type) => ({
  announcement: { name: '📢 ' + t('sub.type.announcement', '新公告'), desc: t('sub.type.announcement.desc', '市政厅发布新公告时通知我') },
  reply:        { name: '💬 ' + t('sub.type.reply', '我的留言被回复'), desc: t('sub.type.reply.desc', '我的市民留言被管理员 / AI 回复时通知我') },
  dm:           { name: '📨 ' + t('sub.type.dm', '新私信'), desc: t('sub.type.dm.desc', '收到新私信时通知我') },
}[type] || { name: type, desc: '' });
...
export async function renderSubCard() {
  ...
  box.innerHTML = `
    <h4 style="margin:0 0 8px 0">🔔 ${t('sub.mySubs', '我的订阅')} (${enabledCount} ${t('sub.enabled', '个开启')})</h4>
    <div class="sub-list" style="display:flex;flex-direction:column;gap:6px">
      ${['announcement', 'reply', 'dm'].map(type => {
        ...
        return `<div style="display:flex;align-items:center;justify-content:space-between;padding:8px;border:2px solid var(--c-stone);background:var(--c-bg-1)">
          ...
          <button class="btn ${enabled ? 'btn-ghost' : 'btn-primary'}" data-type="${type}" data-action="toggle" data-on="${enabled}">
            ${enabled ? '✓ ' + t('sub.subscribed', '已订阅') : '+ ' + t('sub.subscribe', '订阅')}
          </button>
        </div>`;
      }).join('')}
    </div>
    <h4 style="margin:16px 0 8px 0">📬 ${t('sub.recentNotifs', '最近通知')} (${notifs.length})</h4>
    <div class="notif-list" style="display:flex;flex-direction:column;gap:6px">
      ${notifs.length === 0 ? `<p class="muted">${t('sub.noNotif', '还没有通知')}</p>` :
        notifs.map(n => `<div style="display:flex;align-items:flex-start;gap:8px;padding:8px;border:2px solid var(--c-stone);background:${n.read_at ? 'var(--c-bg-2)' : '#fff8d6'};opacity:${n.read_at ? '0.7' : '1'}">
          ...
          ${!n.read_at ? `<button class="btn btn-ghost btn-sm" data-notif="${n.id}" data-action="read">${t('sub.btn.read', '已读')}</button>` : `<span style="font-size:11px;color:var(--c-stone-dark)">${t('sub.read', '已读')}</span>`}
        </div>`).join('')}
    </div>
    ${notifs.some(n => !n.read_at) ? `<button id="notifReadAll" class="btn btn-ghost btn-sm" style="margin-top:8px">${t('sub.readAll', '全部标为已读')}</button>` : ''}
  `;
