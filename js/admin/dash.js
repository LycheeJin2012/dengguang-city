...
import { $, POST, GET, safeRender, t } from './core.js?v=v46-fix-modules';
...
const _TAB_RENDER = {
  ...
  kart:       () => import('./tabs/kart.js').then(m => document.querySelector('.subtabs[data-pane="kart"] .subtab.active')?.dataset.sub === 'circuit' ? m.renderCircuits() : m.renderKarts()),
...
export function bindFilterRadios() {
  document.querySelectorAll('.subtabs').forEach(nav => {
    const activate = (button, render = true) => {
      const pane = nav.dataset.pane;
      nav.querySelectorAll('.subtab').forEach(b => b.classList.toggle('active', b === button));
      document.querySelectorAll(`[data-subview^="${pane}-"]`).forEach(view =>
        view.classList.toggle('active', view.dataset.subview === `${pane}-${button.dataset.sub}`));
      if (render && pane === 'kart' && button.dataset.sub !== 'manage') safeRender(_TAB_RENDER.kart);
    };
    nav.querySelectorAll('.subtab').forEach(b => b.addEventListener('click', () => activate(b)));
    const active = nav.querySelector('.subtab.active');
...
export function renderDash() {
  try {
    ...
    const createAnn = $('#annCreateBtn');
    if (createAnn) createAnn.style.display = a.role === 'super' ? '' : 'none';
  } catch (e) { throw e; }
  showView('dash');
  ...
  if (refresh) refresh.onclick = () => fetchBadges().catch(() => {});
