...
test('incoming urgent and complex tickets are automatically assigned to designated accounts with system audit',()=>fixture(async({DB,ticket})=>{
 ...
 for(const [t,who] of [[urgent,2],[complex,3],[both,2]]){assert.equal(t.http,201);assert.equal(t.dispatch.status,'assigned');const row=await DB.prepare('SELECT assignee_id,priority FROM tickets WHERE id=?').bind(t.id).first();assert.equal(row.assignee_id,who);const event=await DB.prepare("SELECT * FROM ticket_events WHERE ticket_ref=? AND actor_type='system'").bind(String(t.id)).first();assert.equal(JSON.parse(event.details).to,who);assert.ok(JSON.parse(event.details).reason);}
...
test('automatic complaints enforce recusal and super-only assignment',()=>fixture(async({ticket,DB})=>{
 ...
 const no=await ticket('投诉唯一超管',{kind:'admin_complaint',target_admin_id:1});assert.equal(no.dispatch.status,'deferred');assert.equal((await DB.prepare('SELECT assignee_id FROM tickets WHERE id=?').bind(no.id).first()).assignee_id,null);
...
test('repeated concurrent processing cannot assign or emit the event twice',()=>fixture(async({DB,pending,env})=>{const id=await pending();await Promise.all([autoDispatch({env},id),autoDispatch({env},id)]);assert.equal((await DB.prepare("SELECT COUNT(*) AS n FROM ticket_events WHERE ticket_ref=? AND actor_type='system'").bind(String(id)).first()).n,1);assert.equal((await DB.prepare("SELECT COUNT(*) AS n FROM audit_events WHERE resource_id=? AND action='ticket.auto_assigned'").bind(String(id)).first()).n,1);}));
test('manual changes and policy pause win over an in-flight model request',()=>fixture(async({DB,pending,env,call})=>{
 ...
 id=await pending();globalThis.fetch=async()=>{await call('admin/dispatch-settings','PATCH',{enabled:false});return Response.json({choices:[{message:{content:'{"admin_id":2}'}}]});};assert.equal((await autoDispatch({env},id)).status,'skipped');assert.equal((await DB.prepare('SELECT assignee_id FROM tickets WHERE id=?').bind(id).first()).assignee_id,null);
 }finally{globalThis.fetch=original;}
}));
test('only super can change assignment policy and read-only policy views create no audit',()=>fixture(async({call,ticket,DB})=>{assert.equal((await call('admin/dispatch-settings','PATCH',{enabled:false},'wzc')).http,403);assert.equal((await call('admin/dispatch-settings','PATCH',{urgent_admin_id:999})).http,404);assert.equal((await call('admin/dispatch-settings','PATCH',{enabled:false})).http,200);assert.equal((await ticket('紧急事项')).dispatch.status,'paused');const n=async()=>(await DB.prepare('SELECT COUNT(*) AS n FROM audit_events').first()).n;const before=await n();await call('admin/dispatch-settings');assert.equal(await n(),before);}));
test('history changes regular routing after sufficient comparable completed cases; tiny samples do not infer expertise',()=>fixture(async({DB,ticket})=>{
 ...
 const e=await DB.prepare("SELECT details FROM ticket_events WHERE ticket_ref=? AND actor_type='system'").bind(String(t.id)).first();assert.equal(JSON.parse(e.details).learning.completed,4);
}));
test('legacy messages and business submissions also trigger automatic assignment',()=>fixture(async({call,DB})=>{
 const message=await call('messages','POST',{type:'建议',content:'紧急事项',contact:'local'},'player');assert.equal(message.http,201);assert.equal(message.dispatch.status,'assigned');
 ...
 const booking=await call('bookings','POST',{room_id:1,in_date:'2099-01-01',out_date:'2099-01-02',persons:1,name:'citizen',contact:'local'},'player');assert.equal(booking.http,201);assert.equal(booking.dispatch.status,'assigned');assert.ok((await DB.prepare("SELECT assignee_id FROM tickets WHERE source_table='bookings' AND source_id=?").bind(booking.id).first()).assignee_id);
}));
