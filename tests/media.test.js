...
before(async()=>{await ensureDatabase(DB);await DB.prepare("UPDATE dispatch_settings SET enabled=0 WHERE id=1").run();await DB.prepare("INSERT INTO admins(id,username,password_hash,salt,role) VALUES(1,'admin','x','x','super')").run();for(const [id,name] of [[1,'alice'],[2,'bob']])await DB.prepare("INSERT INTO players(id,username,email,password_hash,salt,status) VALUES(?,?,?,'x','x','active')").bind(id,name,name+'@example.invalid').run();for(const [token,p,a] of [['media-alice',1,null],['media-bob',2,null],['media-admin',null,1],['media-combined',1,1]])await DB.prepare("INSERT INTO sessions(token,player_id,admin_id,expires_at) VALUES(?,?,?,'2099-01-01T00:00:00Z')").bind(token,p,a).run();});
after(()=>DB.close());
