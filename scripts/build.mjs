...
for(const name of ['index','hotel','profile','dm','notifications','leaderboard','admin-v37','hotel-owner','404']){const file=root+name+'.html',html=fs.readFileSync(file,'utf8');for(const m of html.matchAll(/(?:href|src)="(\/[^"#?]*)(?:\?[^"#]*)?"/g)){if(m[1]==='/')continue;if(!fs.existsSync(root+m[1].slice(1)))throw new Error(`${name}: missing ${m[1]}`);}}
...
console.log(`Build validated: ${modules.length} JS modules, 9 HTML pages, shared assets. Existing Pages layout preserved.`);
