...
for(const name of ['index','hotel','profile','dm','notifications','admin-v37','hotel-owner','404','knowledge']){const file=root+name+'.html',html=fs.readFileSync(file,'utf8');for(const m of html.matchAll(/(?:href|src)="(\/[^"#?]*)(?:\?[^"#]*)?"/g)){if(m[1]==='/')continue;if(!fs.existsSync(root+m[1].slice(1)))throw new Error(`${name}: missing ${m[1]}`);}}
