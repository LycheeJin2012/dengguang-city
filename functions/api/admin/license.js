...
export async function onRequestGet(context) {
  ...
  if (p.status) { where.push('l.status = ?'); binds.push(p.status); }
  const whereSql = where.length ? 'WHERE ' + where.join(' AND ') : '';
  const rows = await env.DB.prepare(
    `SELECT l.*, p.username AS player_username
     FROM license_signups l LEFT JOIN players p ON p.id = l.player_id
     ${whereSql} ORDER BY l.created_at DESC LIMIT ?`
...
export async function onRequestPatch(context) {
  ...
  await env.DB.prepare('UPDATE license_signups SET status = ? WHERE id = ?').bind(body.status, id).run();
