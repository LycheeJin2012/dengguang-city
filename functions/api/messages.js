...
export const onRequestGet=c=>endpoint(async()=>{
  ...
  const p=await identity(c);const r=await c.env.DB.prepare('SELECT id,name,content,type,status,admin_reply,created_at,replied_at,public_consent,public_visible FROM messages WHERE player_id=? ORDER BY id DESC LIMIT 100').bind(p.id).all();return reply({
    messages:r.results
  }
  );
}
