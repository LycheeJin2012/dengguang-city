...
export const onRequestGet=c=>endpoint(async()=>{
  const u=new URL(c.request.url);if(u.searchParams.get('public')==='1'){
    const rows=await c.env.DB.prepare('SELECT m.id,m.name,m.type,m.content,m.admin_reply,m.created_at,m.replied_at,(SELECT COUNT(*) FROM message_comments x WHERE x.message_id=m.id) AS comment_count FROM messages m ORDER BY m.id DESC LIMIT 100').all();return reply({
