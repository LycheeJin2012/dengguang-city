...
export const onRequestGet=c=>endpoint(async()=>{
 ...
 return reply({places:(await c.env.DB.prepare(`SELECT ${fields}${admin?',published,revision':''} FROM city_places ${admin?'':'WHERE published=1'} ORDER BY id DESC LIMIT 1000`).all()).results});
});
export const onRequestPost=c=>endpoint(async()=>{
 ...
 const category=b.category,dimension=b.dimension,state=b.construction_status;
 if(!['hotel','rail','road','park','facility'].includes(category)||!['overworld','nether','end'].includes(dimension)||!['open','planned','construction','closed'].includes(state))fail(400,'地点分类、维度或施工状态无效');
