...
- v65：人工客服子页与菜单组同名时必须按明确点击目标跳转，不得用记忆子页覆盖。允许管理员只读查看本人客服会话，接入/回复仍需另一位有权限管理员；列表和选中会话自动刷新，不能抹掉未发送草稿。

- v66：用户要求移除榜单展示；保留个人赛道成绩和业务数据。聊天转人工与回复评价合并在同一区域。页面只做纵向滚动，小屏表格使用带可见字段标签的卡片；不得用禁用缩放或裁剪数据代替适配。

- v67：新增我的事务、SUPER 地图与施工维护、本人登录与会话管理及灯灯个人助手；一级导航通知和私信合并为消息。用户授权新增本人安全登录记录，独立保存成功登录时间与粗粒度设备类型，不记 IP/精确位置或浏览行为；原业务审计仍不记录登录。灯灯个人来源必须由服务器当前玩家身份筛选并使用对外字段，禁止题目答案、内部派单、他人记录进入模型。地图草稿仅 SUPER 可读，编辑使用 revision 防止覆盖。

- v68：消息中心通知/私信必须在同一文档内切换；保留聊天草稿与面板，隐藏面板不得自动标记私信已读。用户查看 8884 本地预览时，交付须同步该预览并保留测试数据库。

- v70：用户委托 MiniMax 全站前端布局重构并授权先推送测试、需要时回滚。只回滚前端发布提交，不恢复业务数据库。独立审查必须保持文案、角色权限、消息同页草稿及主世界限制；不得引入按收件人共享的持久草稿。用户正在用的8884预览必须同步。
